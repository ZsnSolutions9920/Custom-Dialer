...
export default function DashboardStats({ agentId }) {
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-5 animate-pulse">
            ...
            <div className="h-7 bg-gray-100 dark:bg-gray-700 rounded w-16" />
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={<PhoneIcon />} label="Total Calls" value={stats.total_calls} subtitle="Last 7 days" />
          <StatCard icon={<ClockIcon />} label="Avg Duration" value={formatDuration(stats.avg_duration)} subtitle="Per answered call" />
          <StatCard icon={<CheckCircleIcon />} label="Answer Rate" value={`${stats.answer_rate}%`} subtitle="Completed calls" />
          <StatCard icon={<ArrowsIcon />} label="In / Out" value={`${stats.inbound_count} / ${stats.outbound_count}`} subtitle="Inbound / Outbound" />
        </div>
      )}
