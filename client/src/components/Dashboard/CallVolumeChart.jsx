  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white rounded-2xl shadow-card border border-gray-100/80 p-6">
        <h3 className="text-sm font-semibold text-gray-800 mb-4">Call Volume</h3>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card border border-gray-100/80 p-6 hover:shadow-card-hover transition-all duration-300">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-xs text-gray-400 font-medium">Last {data.length} days</span>
      </div>
        ...
        {data.map((d) => {
          ...
          return (
            <div key={d.date} className="flex-1 flex flex-col items-center gap-2 group">
              <span className="text-xs font-semibold text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">{d.count}</span>
